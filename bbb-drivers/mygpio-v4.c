//BBB ile interrupt driver (Buton LED yakıyor interrupt sırasında)
...
#define GPIO_PIN 524 //P8_12
#define BUTTON 526 //P8_16
...
static irqreturn_t button_irq_handler(int irq, void *dev_id)
{
    ...
    if (time_before(now, last_irq_time + msecs_to_jiffies(200))) {
        return IRQ_HANDLED;
    }
...
static int __init my_init(void)
{
    ...
    if (gpio_request(BUTTON, "mybutton") < 0) {
        gpio_free(GPIO_PIN);
    ...
    if (request_irq(irq_number, button_irq_handler, IRQF_TRIGGER_RISING, "button_irq", NULL)) {
        gpio_free(BUTTON);
        gpio_free(GPIO_PIN);
